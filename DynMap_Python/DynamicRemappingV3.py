# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 21:24:25 2023
...
def CurOptPotentialPlacement_List_Gen_Hierarchy():
...
def RoutingAvailability_CheckPredecessor_and_Placement():  
    ...
    if not bypassMode: # non-bypass mode
        ...
        # recalculate the start IDX_pd of curOpt in bypass mode
        for pred in predecessors: 
            predOpt1 = pred
            Calculate_CurToPred_Distance()
            temp = placement_dynamic_dict_Opt2PC[pred] + max(xDiff_CurToPred1, yDiff_CurToPred1)
            if temp > IDX_pd:
                IDX_pd = temp
        ...
        while True:
            # recalculate the modulo of IDX_pd
...
def dynamic_placement():
    ...
    for IDX_ps in range( len(placement_static) ):
        ...
        for idx_ps in range( len(placement_static[IDX_ps]) ):
            ...
            CurOptPotentialPlacement_List_Gen_Hierarchy()
