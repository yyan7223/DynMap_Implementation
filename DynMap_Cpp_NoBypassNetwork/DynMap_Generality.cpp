...
void PlacementDynamic_Record(Tile tile){
    ...
    placement_dynamic_tile_occupy[IDX_pd_modulo][tileId] = 1;
    placement_dynamic_dict_Opt2Tile[curOpt] = tile;
    placement_dynamic_dict_Opt2PC[curOpt] = IDX_pd;
    placement_done[curOpt] = 1;
} 

// supplement the inport and outport occupation status of the given tiles
void Supplement_IO_Port_Occupation(int srcPC, Tile sourceTile, Tile targetTile){
    int xDiff = targetTile.X - sourceTile.X;
    int yDiff = targetTile.Y - sourceTile.Y;
    int dirInIdx = XbarIn_Direction(xDiff, yDiff);
    int dirOutIdx = XbarOut_Direction(xDiff, yDiff);
    int srcPC_modulo = srcPC % DynamicPlacement_II;
    int tgtPC_modulo = (srcPC + 1) % DynamicPlacement_II;
    placement_dynamic_IO_occupy[tgtPC_modulo][targetTile.ID][dirInIdx-1] = 1;
    placement_dynamic_IO_occupy[srcPC_modulo][sourceTile.ID][dirOutIdx+8-1] = 1;
}

void save_status(){
    for(int i = 0; i < bypassOverlapPages; i++){
        for(int j = 0; j < maxPCs; j++){
            for(int k = 0; k < cgra_size * cgra_size; k++){
                if(placement_dynamic_bypass_backup[i][j][k] != "" && placement_dynamic_bypass[i][j][k] != ""){
                    placement_dynamic_bypass_backup[i][j][k] = placement_dynamic_bypass[i][j][k];
                }
            }
        }  
    }

    memcpy(placement_dynamic_IO_occupy_backup, placement_dynamic_IO_occupy, DynamicPlacement_II * cgra_size * cgra_size * 16 * sizeof(int));
    placement_dynamic_dict_Opt2Tile_backup = placement_dynamic_dict_Opt2Tile;
    placement_dynamic_dict_Opt2PC_backup = placement_dynamic_dict_Opt2PC;
    placement_done_backup = placement_done;
    dependency_successor_backup = dependency_successor;
    dependency_predecessor_backup = dependency_predecessor;
}
...
void recover_status(){
    for(int i = 0; i < bypassOverlapPages; i++){
        for(int j = 0; j < maxPCs; j++){
            for(int k = 0; k < cgra_size * cgra_size; k++){
                if(placement_dynamic_bypass[i][j][k] != "" && placement_dynamic_bypass_backup[i][j][k] != ""){
                    placement_dynamic_bypass[i][j][k] = placement_dynamic_bypass_backup[i][j][k];
                }
            }
        }  
    }

    memcpy(placement_dynamic_IO_occupy, placement_dynamic_IO_occupy_backup, DynamicPlacement_II * cgra_size * cgra_size * 16 * sizeof(int));
    placement_dynamic_dict_Opt2Tile = placement_dynamic_dict_Opt2Tile_backup;
    placement_dynamic_dict_Opt2PC = placement_dynamic_dict_Opt2PC_backup;
    placement_done = placement_done_backup;
    dependency_successor = dependency_successor_backup;
    dependency_predecessor = dependency_predecessor_backup;
}
...
int XbarIn_Direction(int xDiff, int yDiff){
...
int XbarOut_Direction(int xDiff, int yDiff){
    // N:1 S:2 E:3 W:4 NE:5 SE:6 NW:7 SW:8
    if(xDiff < 0){
        if(yDiff < 0) return 7;
...
bool BypassOptPlacement_Gen_Record(){ // record the dynamic placement of bypass operation
    ...
    for(auto stride : bpsStride_architecture[architecture]){
        ...
        if((xDiff != 0) || (yDiff != 0)){ // actually move
            // check whether Tile is legal
            bool isAllocatedTiles = false;
            // bool notForbiddenTiles = false;
            if (find(allocated_tiles.begin(), allocated_tiles.end(), xy2Tile[x][y]) != allocated_tiles.end()) isAllocatedTiles = true;
            // if (find(bypassForbiddenTiles.begin(), bypassForbiddenTiles.end(), xy2Tile[x][y]) == bypassForbiddenTiles.end()) notForbiddenTiles = true;
            // if (isAllocatedTiles && notForbiddenTiles) break;
            if (isAllocatedTiles) break;
        }
    }
    ...
    while(true){
        ...
        int formerPC_modulo = (IDX_pd_bypass_modulo == 0) ? (DynamicPlacement_II - 1) : (IDX_pd_bypass_modulo - 1); // avoid negative array index
        int inportConflict = placement_dynamic_IO_occupy[IDX_pd_bypass_modulo][tileId][dirInIdx-1];
        int outportConflict = placement_dynamic_IO_occupy[formerPC_modulo][bypassSrcTile.ID][dirOutIdx+8-1];
        if((!inportConflict && !outportConflict) == true){ // both requested inport and outport are not occupied
            ...
            while (true) // finish the placement of bypassOpt
            {
                if(placement_dynamic_bypass[page][IDX_pd_bypass][tileId] == ""){
                    ...
                    placement_dynamic_IO_occupy[IDX_pd_bypass_modulo][tileId][dirInIdx-1] = 1;
                    placement_dynamic_IO_occupy[formerPC_modulo][bypassSrcTile.ID][dirOutIdx+8-1] = 1;
...
bool bypassOptGen_and_Placement(string srcOpt, string tgtOpt){
    ...
    while (true)
    {
        ...
        if(SrcToTgt_Distance_Satisfy_Topology_BypassMode()){ //check if satisfy topology
            // bypass mode, supplement the I/O port occupation status of the the last bypassSrcTile and bypassTgtTile, respectively
            // Supplement_IO_Port_Occupation(placement_dynamic_dict_Opt2PC[bypassSrcOpt], bypassSrcTile, bypassTgtTile); 
            return true; 
        }
    } 
}
...
bool RoutingAvailability_CheckPredecessor_and_Placement(){
    ...
    for(auto tile : curOptPotentialPlacement){
        ...
        vector<int> dirInIdxs;
        vector<int> dirOutIdxs;
        // bypassForbiddenTiles.clear();
        for(auto pred : predecessors){
            predOpt1 = pred;
            predTile1 = placement_dynamic_dict_Opt2Tile[predOpt1];
            curTile = tile;
            Calculate_CurToPred_Distance();
            dirInIdxs.push_back(XbarIn_Direction(xDiff_CurToPred1, yDiff_CurToPred1));
            dirOutIdxs.push_back(XbarOut_Direction(xDiff_CurToPred1, yDiff_CurToPred1));
            if(!CurToPred_Distance_Satisfy_Topology()){
                bypassPreds.push_back(pred);
                bypassMode = true;
            }
            // else{
            //     bypassForbiddenTiles.push_back(predTile1);
            // }
        }
        /////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(!bypassMode){ // non-bypass mode, IDX_pd and IDX_pd_modulo are initial value
            ...
            save_status(); // record before entering bypass mode
            int IDX_pd_bypass_MAX = initial_IDX_pd;
            placement_dynamic_dict_Opt2Tile[curOpt] = tile;
            for(auto pred : bypassPreds){
                int IDX_pd_lastBypass = -1;
                if(!bypassOptGen_and_Placement(pred, curOpt)){
                    // return false; // go to next II
                    recover_status(); // recover status if failed
                    next_tile = true; // move to next tile in the recommendation list
                    break;
                } 
                else IDX_pd_lastBypass = IDX_pd_bypass;
                if(IDX_pd_lastBypass > IDX_pd_bypass_MAX) IDX_pd_bypass_MAX = IDX_pd_lastBypass;
            }
            if(next_tile) continue; // move to next tile in the recommendation list
        ...
        // try to finish placement of curOpt at current tile
        while(true){
            if (placement_dynamic_tile_occupy[IDX_pd_modulo][tile.ID] == 0){ // not occupied
                bool portNoConflict = false;
                for(int i = 0; i < predecessors.size(); i++){ // inport and outport not conflict
                    int srcPC = IDX_pd_modulo;
                    int tgtPC = (srcPC + 1 >= DynamicPlacement_II) ? 0 : (srcPC + 1);
                    Tile predTile = placement_dynamic_dict_Opt2Tile[predecessors[i]];
                    int outportConflict = placement_dynamic_IO_occupy[srcPC][predTile.ID][dirOutIdxs[i]+8-1];
                    int inportConflict = placement_dynamic_IO_occupy[tgtPC][curTile.ID][dirInIdxs[i]-1];
                    if((!inportConflict && !outportConflict)){
                        portNoConflict = true;
                        break;
                    }
                }
                if(portNoConflict){
                    for(int i = 0; i < predecessors.size(); i++){ // occupy inport and outport
                        int srcPC = IDX_pd_modulo;
                        int tgtPC = (srcPC + 1 >= DynamicPlacement_II) ? 0 : (srcPC + 1);
                        Tile predTile = placement_dynamic_dict_Opt2Tile[predecessors[i]];
                        placement_dynamic_IO_occupy[srcPC][predTile.ID][dirOutIdxs[i]+8-1] = 1;
                        placement_dynamic_IO_occupy[tgtPC][curTile.ID][dirInIdxs[i]-1] = 1;
                    }
...
void CurOptPotentialPlacement_List_PartialInherit_Gen(){
    ...
    for (auto Tile : allocated_tiles_levels_dynamic[dynamic_level]){ // same Tile in same level (little trick)
        if(Tile.ID == curTileStatic.ID) curOptPotentialPlacement.push_back(Tile);
    }

    for (auto Tile : allocated_tiles_levels_dynamic[dynamic_level]){ // other Tile in same level
        if(Tile.ID != curTileStatic.ID) curOptPotentialPlacement.push_back(Tile);
    }
...
void Reset(string kernel, string shape){
    ...
    placement_done_backup = placement_done_kernels[kernel];
    ...
    dependency_predecessor_backup = dependency_predecessors_kernels[kernel];
    dependency_successor = dependency_successors_kernels[kernel];
    dependency_successor_backup = dependency_successors_kernels[kernel];
    ...
    placement_dynamic_dict_Opt2PC_backup.clear(); placement_dynamic_dict_Opt2Tile_backup.clear();
    memset(placement_dynamic, 0, sizeof(placement_dynamic));
    memset(placement_dynamic_bypass, 0, sizeof(placement_dynamic_bypass));
    memset(placement_dynamic_bypass_backup, 0, sizeof(placement_dynamic_bypass_backup));
    memset(placement_dynamic_tile_occupy, 0, sizeof(placement_dynamic_tile_occupy));
    memset(placement_dynamic_IO_occupy, 0, sizeof(placement_dynamic_IO_occupy));
    memset(placement_dynamic_IO_occupy_backup, 0, sizeof(placement_dynamic_IO_occupy_backup));
}
...
int main(){
    ...
    for(auto kernel : test_kernels){
        ...
        for(auto shape : test_shapes[kernel]){
            ...
            for(int i = 0; i < 10; i++){ // run for 10 times
                ...
                while (true)
                {
                    ...
                    if(success){
                        ...
                        else threshold += 5; // stride = 1, best quality. stride = 8, best speed
                    }
                }
            ...
            cout<<"TestCase:["<<test_case<<"/576] "<<kernel<<" "<<shape<<" mapping done, avgTimeConsump is "<<tSum_10times/10.0;
