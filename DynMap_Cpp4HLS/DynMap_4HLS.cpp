...
bool BypassOptPlacement_Gen_Record(){ // record the dynamic placement of bypass operation
    ...
    while(true){
        ...
        if((!inportConflict && !outportConflict) == true){ // both requested inport and outport are not occupied
            char page = 0; // to record overlapped bypassOpt that using different inport and output
...
void record_mappingResults(char kernel, char shape){
    ofstream dataFile;
    string fileName = "cpp mapping results//Kernel" + to_string(kernel) + "_Shape" + to_string(shape) + ".txt";
    dataFile.open(fileName, ofstream::out | ofstream::trunc);
    for(char i = 0; i < DFG_NodesCount_kernels_values[kernel_idx]; i++){ // print mapping results
        dataFile<<"DFGNode "<<int(placement_static_kernels_values[kernel_idx][i])<<", PC="<<int(placement_dynamic_dict_Opt2PC_values[i])<<", Tile="<<int(placement_dynamic_dict_Opt2Tile_values[i])<<endl;
    }
    dataFile<<"Achieved DynamicPlacement_II is "<<int(DynamicPlacement_II)<<endl;
    dataFile<<"threshold is "<<int(threshold);
    dataFile.close();
}
...
void compare_mappingResults_withPython(){
    ...
    for(char i = 0; i < kernelNum; i++){
        for(char j = 0; j < shapeNumPKernel; j++){
            char cppResults, pythonResults;
            string cppFileName, pythonFileName;
            kernel_shape_idx_generator(i, j);
            cppFileName = "cpp mapping results//" + to_string(test_kernels[kernel_idx]) + " " + to_string(test_shapes_values[shape_idx]) + ".txt";
            pythonFileName = "python mapping results//" + to_string(test_kernels[kernel_idx]) + " " + to_string(test_shapes_values[shape_idx]) + ".txt";
            op.open(cppFileName);
            ...
            if(cppResults == pythonResults) cout<<"Comparing C++ mapping results of Kernel "<<int(test_kernels[kernel_idx])<<", Shape "<<int(test_shapes_values[shape_idx])<<" to python, same"<<endl;
            else cout<<"Comparing C++ mapping results of Kernel "<<int(test_kernels[kernel_idx])<<", Shape "<<int(test_shapes_values[shape_idx])<<" to python, different"<<endl;
        }
    }
}
...
int main(){
    ...
    for(char i = 0; i < kernelNum; i++){
        for(char j = 0; j < shapeNumPKernel; j++){
            ...
            record_mappingResults(test_kernels[kernel_idx], test_shapes_values[shape_idx]);

            cout<<"TestCase:["<<int(test_case)<<"/72], Kernel "<<int(test_kernels[kernel_idx])<<", Shape "<<int(test_shapes_values[shape_idx])<<", II="<<int(DynamicPlacement_II)<<endl;
            test_case += 1;
        }
    }

    // after all test cases finished, compare mapping results with python
    // compare_mappingResults_withPython(); // use Dynmap_Cpp project to compare the results, cause all DFGNodes' names are converted to char
