...
void analyze_static_levels_distribution(string kernel){
    ...
    for(int i = 0; i < 25; i++){ // static mapping results is 5x5 CGRA
        TilesUsedTimes[i] = 0;
    }
    for(auto staticOpt : placement_static){
    ...
    // cout<<"In static mapping result of kernel "<<kernel<<" under 5x5 CGRA, Level_0 Tiles used ["<<numUsedTiles_level_0<<"/9], Level_1 Tiles used ["<<numUsedTiles_level_1<<"/12], Level_2 Tiles used ["<<numUsedTiles_level_2<<"/4]"<<endl;
}

int main(){
    architecture = "Diagonal"; // Diagonal, Orthogonal
    heterogenous = false; // true, false
    ...
    for(auto kernel : test_kernels){
        ...
        for(auto shape : test_shapes[kernel]){
            ...
            // cout<<"TestCase:["<<test_case<<"/18]                    "<<tSum_10times/10.0<<"                    "<<IISum_10times/10.0<<endl;
